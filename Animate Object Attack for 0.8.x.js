...
//Ac check target
    ...
    for (atks = 0; atks < attacksAos; atks++) {
        let r1 = new Roll(aoHitRoll).roll({async: false});
        if (r1.result == 1){
...
//Chat Update and Attackroll
    ...
    if (dmAcCheck == 1){
        let acRoll = new Roll(`${enemieAC}`).roll({async: false}).toMessage({flavor: flavor, speaker } ,{ rollMode: 'blindroll'});
    };   
    ...
    let damageRoll = new Roll(`${nAtkDice * aosDieN}${aosDie} +${nAtkbns}`).roll({async: false});
